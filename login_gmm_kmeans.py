...
class KeyStroke():
    ...
    def __init__(self,file_path,threshold = 1.50, Kmeans = False,GMM = False , n_patterns = 20, debug = False, update = True):
    ...
    def _recognition(self, Xp, yp):
        ...
        if self.n_current != self._k and self.n_current < self.n_patterns * 0.5:
        ...
        input_score, _ = self._get_diff(Xp[:-self.n_valid],yp,weights = weights)
        if self.debug:
    ...
    def _get_diff(self,X,y,weights=None):
        if weights is None:
    ...
    def _K_means(self, X, k = 4):
        ...
        for i in range(k,len(X)-self.n_valid):
            _, j = self._get_diff(init_X,X[i])
            init_X[j] = init_X[j]*num_X[j]/(num_X[j] + 1) + X[i]/(num_X[j] + 1)
            num_X[j] += 1
            
        for i in range(self.n_valid):
            init_X = np.vstack([init_X,X[-i].reshape(1,len(X[-i]))])

        self.n_current = k + self.n_valid
        self.n_patterns = k + self.n_valid
        self._k = k
...
if __name__ == '__main__':
    # n_patterns of train is 20
    # n_patterns of train2 is 50
    ...
    for i in range(n):
        ...
        ret,ret1,ret2 = validation(train, test,threshold=threshold,Kmeans = True,GMM=False, debug = False, n_patterns = 20)
    ...
    train = "./train2/"      # n_patterns = 50, threshold = 2
    k1 = KeyStroke(train, debug = True,threshold = 2.5,n_patterns = 50,Kmeans = True,update = False,GMM=False)
    while True:
        k1.login()
        sys.stdout.flush()
    '''
